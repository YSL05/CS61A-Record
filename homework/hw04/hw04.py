...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return (length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m)))) and ((not is_mobile(end(left(m)))) or (balanced(end(left(m))))) and ((not is_mobile(end(right(m)))) or balanced(end(right(m))))
